...
cartesian_product = [list([x,y]) for x in ['A','B','C'] for y in [1,2,3]]
...
LofL_sum = sum([sum(x) for x in LofL]) #sum(sum(LofL,[]))
...
list_sum_zip = [sum(j) for j in zip([10, 25, 40],[1, 15, 20])]
...
listdict2dict = {names[i]:id2salary[j] for i in range(len(names)) for j in id2salary.keys() if i==j}
