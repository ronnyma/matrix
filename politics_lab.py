...
def create_voting_dict(strlist):
    ...
    for x in strlist:
        y = x.split()
        d.update({y[0]: [int(x) for x in y[3:]]})
...
def policy_compare(sen_a, sen_b, voting_dict):
    ...
    """
    return sum(p*q for p,q in zip(voting_dict[sen_a], voting_dict[sen_b]))
...
def most_similar(sen, voting_dict):
    ...
    for x in voting_dict.keys():
        if x != sen:
            tmp = policy_compare(sen, x, voting_dict)
            if tmp > ms:
                ms = tmp
                ms_sen = x
...
def find_average_similarity(sen, sen_set, voting_dict):
    ...
    avg = 0
    for x in sen_set:
        avg += policy_compare(sen, x, voting_dict)

    return avg / len(sen_set)
...
def find_average_record(sen_set, voting_dict):
    ...
    Example: 
        ...
        [0.0, 1.0, 1.0]
    """
    first_element = next( iter( voting_dict.keys() ) )
    h = [0 for x in range(len(voting_dict[first_element]))]
    for s in sen_set:
        h = [x+y for x,y in zip(voting_dict[s], h )]
    a = len(sen_set) 
    return [n/a for n in h]
...
def bitter_rivals(voting_dict):
    ...
    br = ()
    curr = 9999
    f = open('bitter_file', 'a')

    for x in voting_dict.keys():
        for y in voting_dict.keys():
            if x != y:
                tmp = policy_compare(x, y, voting_dict)
                f.write(str(tmp))
                f.write("\t")
                f.write(x)
                f.write(y)
