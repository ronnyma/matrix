...
def equal(u,v):
    ...
    if len(set(i.keys()) - set(j.keys())) or len(set(i.keys()) - set(j.keys())):
        return False

    if all([i[x] == j[y] for x in i.keys() for y in j.keys() if x == y and (i[x] != 0 or j[y] != 0)]):
        return True
    else:
        return False
...
def scalar_mul(v, alpha):
    ...
    if alpha == 0:
        ...
        return Vec(v.D,{x:v.f[x]*alpha for x in v.f.keys()})
...
def neg(v):
    ...
    return Vec(v.D,{x:-v.f[x] for x in v.f.keys()})
