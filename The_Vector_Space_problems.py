...
def vec_select(veclist, k):
    ...
    True
    '''
    return [x for x in veclist if (k not in x.f or x[k] == 0)]
...
def vec_sum(veclist, D):
    ...
    s = Vec(D, {})
    for d in D:
        s.f.update({d:sum([x.f[d] for x in veclist if d in x.f.keys()])})
...
def GF2_span(D, S):
    ...
    for i in range(n):
        z = "{:0b}".format(i).zfill(n)
        span.add(sum([int(z[-1-j])*s for j,s in enumerate(S)]))
